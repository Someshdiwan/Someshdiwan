...
<svg xmlns="http://www.w3.org/2000/svg"
  ...
  <defs>
    ...
    <linearGradient id="cardGrad" x1="0" x2="1" y1="0" y2="1">
      ...
      <stop offset="100%" stop-color="#ffe49a"/>
    </linearGradient>

    <linearGradient id="numGrad" x1="0" x2="0" y1="0" y2="1">
      <stop offset="0%" stop-color="#e09b2a"/>
      <stop offset="100%" stop-color="#b96518"/>
    </linearGradient>

    <style>
      .card-font { font-family: "Permanent Marker","Comic Sans MS","Segoe UI",Roboto,Arial,sans-serif; -webkit-font-smoothing:antialiased; }
      .title { fill:#3f3218; font-weight:700; font-size:20px; text-anchor:middle; letter-spacing:0.6px; }
      .big { fill:url(#numGrad); font-weight:900; font-size:76px; text-anchor:middle; filter: drop-shadow(0 6px 10px rgba(0,0,0,0.28)); }
      .sub { fill:#4f4b4b; font-size:16px; text-anchor:middle; letter-spacing:0.4px; }
  ...
  <!-- subtle overall background (keeps README viewer from showing tight edges) -->
  ...
  <g filter="url(#cardShadow)">
    ...
    <path d="M${cardX + cardW - 48} ${cardY + 32} q-8 14 -22 20" stroke="#efd59a" stroke-width="1.2" fill="none" opacity="0.7"/>
    <ellipse cx="${cardX + cardW - 80}" cy="${cardY + 20}" rx="7" ry="3.5" fill="#fff9e0" opacity="0.85"/>
  </g>

  <!-- clip everything that should be kept inside the card -->
  <g clip-path="url(#cardClip)">

    <!-- left flame + egg shadow (entirely inside clip so won't overflow) -->
    <g transform="translate(${cardX + 36}, ${cardY + 46})">
      ...
      <g class="flame-anim" transform="translate(0,-6) scale(0.92)">
        <g class="flame-flicker">
          <path d="M76 18 C56 -4 42 -4 28 18 C16 36 20 82 54 88 C88 94 102 56 76 18 Z" fill="#f0b93d"/>
          <path d="M64 48 C56 36 50 36 42 48 C38 56 46 68 60 64 C70 62 80 56 64 48 Z" fill="#fff3d8" opacity="0.95"/>
          <path d="M58 10 C52 4 44 6 40 12 C38 18 44 24 52 22 C58 20 64 14 58 10 Z" fill="#ffea9e" opacity="0.96"/>
        </g>
      </g>
    </g>

    <!-- main texts, centered inside card area -->
    <g class="card-font">
    ...
    <!-- small top-right accent flame now fully inside clip so any overflow is removed -->
    <g transform="translate(${cardX + cardW - 92}, ${cardY + 50}) scale(0.52)" class="flame-anim">
      ...
      <path d="M18 -6 C12 -16 -4 -18 -10 -6 C-14 2 -10 22 6 24 C20 26 26 10 18 -6 Z" fill="#f0b93d"/>
